        ...
        $("#left" + i).live("click",
            function (e) {
                ...
                if (commander.left.active && commander.left.selected == n)
                    commander.delve();
                else { commander.left.info = false; commander.left.active = true; commander.right.active = false; commander.left.selected = n; commander.draw(); }
            }
        )
        $("#rite" + i).live("click",
            function (e) {
                ...
                if (commander.right.active && commander.right.selected == n)
                    commander.delve();
                else { commander.right.info = false; commander.right.active = true; commander.left.active = false; commander.right.selected = n; commander.draw(); }
            }
        )
    }
    ...
    for (i = 0; i < panelheight; i++) {
        $("#" + i).live("click",
            function (e) {
                ...
                if (menu.selection == n)
                    menu.dispatch(e);
                else { menu.selection = n; menu.show(); }
            }
        )
    }
